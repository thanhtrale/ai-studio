...
This is not hypothetical. A real run left `sd-server.exe` holding 444 MiB of a
16 GiB card after its parent was terminated: `atexit` does not run when Windows
...
from arm_qwen_edit.jobobject import KillOnClose
...
def test_a_child_in_the_job_still_runs_normally() -> None:
    ...
    child = subprocess.Popen(
        [sys.executable, "-c", "print('alive')"], stdout=subprocess.PIPE
    )
    try:
        job.adopt(child)
